...
The repo contains one canonical Term bank with two audience Tracks: `coding` and `agent-systems`. Do not duplicate a Term between Tracks. Add the appropriate Track values to its frontmatter instead.

Every entry must also declare:

- `category`: one of the category slugs defined in `internal/term-metadata.ts`
- `tracks`: one or both supported Tracks
- `term_status`: `established`, `emerging`, `vendor`, `project`, or `heuristic`
- `level`: `foundational`, `intermediate`, or `advanced`

Run `npm run check` after changing Terms, metadata, the Curriculum, or the README generator.
